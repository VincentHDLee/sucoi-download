...
class SucoiAppController:
    ...
    def add_urls_to_download_queue(self, urls, platform):
        ...
        def do_add():
            ...
            for url in urls:
                ...
                # Generate unique ID (Platform_HashOfURL might be better)
                import hashlib
                ...
                try:
                    # TODO: Maybe fetch title async? For now, use URL part
                    video_title = url.split('/')[-1] if '/' in url else url
                    if not video_title: video_title = url # Fallback
                    # Store full URL in description column for retrieval
            ...
            status_message = f"添加了 {added_count} 个任务。"
            if skipped_count > 0: status_message += f" 跳过 {skipped_count} 个。"
    ...
    def start_selected_downloads(self):
        ...
        self.update_status(f"开始准备下载 {len(selected_iids)} 个任务...")
    ...
    def _monitor_download_threads(self):
        ...
        def final_ui_update():
            ...
            # TODO: Could check treeview for final counts of success/error
            self.update_status(final_status_msg)
            # Optionally show a summary messagebox
            self.show_message("下载状态", final_status_msg)
