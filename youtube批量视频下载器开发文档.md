...
### 功能需求与设计
**当前 GUI 设计 (v0.2.0):**
- **状态栏：** 显示当前操作状态（就绪、搜索中、下载中、完成、错误等）。
- **设置按钮：** 打开设置窗口。
- **搜索框：** 用户输入搜索关键词。
- **搜索按钮：** 触发 YouTube 视频搜索。
- **URL 输入/结果框：** 用户可手动输入视频 URL（每行一个），或显示搜索结果（自动填充）。
- **保存路径：** 显示当前下载路径，可通过按钮选择新路径。
- **选择路径按钮：** 打开文件夹选择对话框以更改当前下载路径。
- **开始下载按钮：** 启动下载流程（基于 URL 输入/结果框中的内容）。
